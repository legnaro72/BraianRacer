...
class BrainUI {
  ...
  header() {
    ...
    return `<header class="nav"><button class="brand" data-action="NAV" data-page="HOME">
      ...
      <nav aria-label="Navigazione principale">${[['HOME','Garage'],['LEADERBOARD','Classifica'],['HELP','Gioca'],['DEDICATIONS','Dediche ♥'],['PHOTOS','Foto ♥'],['SUPERVISOR','🔐 Area Sposi']].map(([page,label])=>
  ...
  render(view) {
    ...
    if(view==='MULTIPLAYER')return this.intro('MULTIPLAYER · IN PROGRESS','Stiamo preparando la sfida di gruppo.','La modalità è temporaneamente disabilitata mentre ne miglioriamo stabilità e fluidità.')+this.back();
  ...
  home(welcome) {
    ...
    return `<section class="hero"><div class="hero-copy"><span class="eyebrow"><span class="pulse"></span> IRENE & DANIELE · UNA VITA A TUTTO GAS</span>
      ...
      `<div class="welcome-back"><span class="avatar small">${esc(d.player.nickname.slice(0,2).toUpperCase())}</span><span>Bentornato, <strong>${esc(d.player.nickname)}.</strong> La festa ti aspetta!</span><button class="text-link switch-player-home" data-action="LOGOUT">⇄ Cambia giocatore</button></div><div class="dedication-invite"><span class="dedication-heart">♥</span><div><h3>Un pensiero per gli sposi</h3><p>Il tuo augurio diventa un ricordo da conservare.</p></div>${button("Lascia una dedica", "NAV", "primary", 'data-page="DEDICATIONS"')}</div>`}</div>${heroArt()}</section>
  ...
  help() {
    return this.intro('SCEGLI LA TUA SFIDA.','Poi parti subito.')+
      `<section class="mode-section"><div class="section-heading"><h2><span class="section-number">01</span> Come vuoi giocare?</h2><span>IL PROSSIMO TRAGUARDO INIZIA QUI</span></div>
      <div class="mode-grid"><button class="mode-card single" data-action="START_SINGLE"><div class="mode-top"><span class="feature-icon">${icons.car}</span><span class="pill">1 GIOCATORE</span></div><div class="mode-bottom"><div><h3>Partita singola</h3><p>Accompagna gli sposi verso il prossimo traguardo.</p></div><span class="circle-arrow">${icons.arrow}</span></div><div class="mode-track"></div></button>
      <button class="mode-card multi" disabled aria-disabled="true"><div class="mode-top"><span class="feature-icon">${icons.people}</span><span class="pill in-progress">IN PROGRESS</span></div><div class="mode-bottom"><div><h3>Multiplayer</h3><p>Temporaneamente non disponibile.</p></div><span class="circle-arrow">•••</span></div><div class="mode-track"></div></button></div></section>
      <h2 class="subheading">Come si gioca</h2><div class="help-grid">${[['01','Guida e sopravvivi.','Muoviti con ← → oppure A e D. Tieni premuto ↑, W o Shift per accelerare fino a 1,6 volte la velocità; rilascia per rallentare. Su smartphone usa i pulsanti o trascina il dito sulla pista e tieni premuto ACCELERA per aumentare la velocità. Parti con 3 vite: ogni urto ne costa una, poi hai 1,3 secondi di protezione.'],['02','Attraversa il traguardo.','Tocca la pista, premi F o tocca ✿ BOUQUET per lanciare fiori: mira ai palloncini a cuore, ogni colpo riuscito vale +2 punti. Puoi tenere premuto. Supera i gruppi di ostacoli fino al termine del percorso. Le stelle valgono +1, lo scudo assorbe un urto e il bonus tempo rallenta la strada per 4 secondi.'],['03','Pensa veloce.','Rispondi a 3 domande, con 15 secondi per ognuna. Risposta corretta: +1. Sbagliata o tempo scaduto: −2. I punteggi negativi sono possibili, ma gli errori al quiz non tolgono vite.']].map(([n,h,p])=>`<section class="panel"><span class="step-number">${n}</span><h2>${h}</h2><p>${p}</p></section>`).join('')}</div>
      <section class="panel help-multi multiplayer-progress"><span class="feature-icon lavender">${icons.people}</span><div><span class="pill in-progress">IN PROGRESS</span><h2>Multiplayer in lavorazione.</h2><p>La modalità di gruppo è temporaneamente disabilitata mentre ne miglioriamo stabilità e fluidità. La partita singola è disponibile normalmente.</p></div></section>`+this.back();
  }
  lobby() {
  ...
  click(event) {
    ...
    if(action==='LOGOUT'){
      ...
      this.signature='';this.mountView('LOADING');this.send('LOGOUT');return;
    }
    this.audio.unlock();
