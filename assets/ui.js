  ...
  header() {
    ...
    return `<header class="nav"><button class="brand" data-action="NAV" data-page="HOME">
      ...
      <nav aria-label="Navigazione principale">${[['HOME','Garage'],['LEADERBOARD','Classifica'],['HELP','Come si gioca'],['DEDICATIONS','Dediche ♥'],['PHOTOS','Foto ♥']].map(([page,label])=>
        `<button data-action="NAV" data-page="${page}" class="nav-link ${(this.menuOverlay||d.page)===page?'selected':''}" ${!d.player?'disabled':''}>${label}</button>`).join('')}<a class="nav-link manual-link" href="app/static/manuale-brain-racer.pdf" target="_blank" rel="noopener">Manuale ↗</a></nav>
      ...
      '<span class="edition">ARCADE / VOL. 01</span>'}</div></header><nav class="mobile-nav" aria-label="Menu smartphone">${d.player?[['HOME','Garage'],['LEADERBOARD','Classifica'],['HELP','Come si gioca'],['DEDICATIONS','Dediche ♥'],['PHOTOS','Foto ♥']].map(([page,label])=>`<button data-action="NAV" data-page="${page}" class="${(this.menuOverlay||d.page)===page?'selected':''}">${label}</button>`).join(''):''}<a class="manual-link" href="app/static/manuale-brain-racer.pdf" target="_blank" rel="noopener">Manuale ↗</a></nav>${active&&this.menuOverlay?`<div class="active-game-banner"><span>Partita in corso</span>${button('Riprendi subito','RESUME','primary')}</div>`:''}`;
  }
